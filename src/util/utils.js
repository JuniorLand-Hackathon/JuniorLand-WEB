export function extractVideoIDFromURL(url) {
    const regExp = /^(?:https?:\/\/)?(?:m\.|www\.)?(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&v=))(?<videoId>(\w|-){11})(?:\S+)?$/;
    ...
    switch (randNum) {
        case 1:
            if (giftIndex <= giftsSize)
                return { ...data, link: `/present/${giftIndex++}` };
            else {
                return { ...data, link: '/404' };
            }
        case 2:
            if (educationIndex <= educationsSize)
                return { ...data, link: `/education/${educationIndex++}` };
            else {
                return { ...data, link: '/404' };
            }
